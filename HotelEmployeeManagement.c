...
// external data types
/////////////////////////////////////////
extern int NUM_OF_HOTEL_EMP; // Change this i into number of employee in your progrm
extern HotelEmployee Hem[HOTELEMPLOYEE_NUM];
/////////////////////////////////////////////////////////////////
...
void add_Hotelemployee()
{
		// open the file to write data into it
	FILE *fp = fopen("HotelEmployeeDB.txt", "a");
	...
	printf("Enter the salary "
		"of employee\n");
	scanf("%d", &Salary);
	printf("Enter the Hotel section ID"
...
void find_fn_Hotel_emp()
{
	char Hotelempfname[50];
	printf("Enter the First Name of the Employee\n");
	scanf("%s", Hotelempfname);

	FILE *fp = fopen("HotelEmployeeDB.txt", "r");
	...
	while (fgets(line, sizeof(line), fp))
	{
		...
		if(!strcmp(fName, Hotelempfname))
		{
			printf("The Hotel Employee Details are\n");
...
// by the roll number
void find_rl_Hotel_emp()
{
	int temp;
	printf("Enter the Roll Number of the Hotel Employee\n");
	scanf("%d", &temp);
...
void find_Hotel_s()
{
	...
	while (fgets(line, sizeof(line), fp))
	{
		...
		char * roll = strtok(NULL, ",");
		char * salary = strtok(NULL, ",");
		int SectionID[HOTEL_SECTION_NUM];
		int found = 0;
		for (int i = 0; i < 2; i++)
		{
			char * sid = strtok(NULL, "|");
...
void tot_Hotel_e()
{
	...
	printf("The total number of Hotel employees is %d\n",counter);
	fclose(fp);
}
...
void del_Hotel_e()
{
	...
	fclose(fp);
	fclose(temp);
	remove("HotelEmployeeDB.txt");
	rename("temp.txt","HotelEmployeeDB.txt");
		...
		if (atoi(roll) == x) {
			...
			case 5:
				...
				for (int i = 0; i < 2; i++) {
					int sectionID;
					scanf("%d", &sectionID);
					SectionID[HOTEL_SECTION_NUM+i] = sectionID;
				}
